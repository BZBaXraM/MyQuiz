...
public class Menu
{
    ...
    public Menu()
    {
        ...
        using (FileStream fileStream =
               new FileStream(PathAccs, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite))
        {
        ...
        using (FileStream fileStream =
               new FileStream(PathStats, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite))
        {
            using (StreamReader reader = new StreamReader(fileStream, Encoding.Default))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    Statistics.Add(new KeyValuePair<string, string>(matchQuizzez.Value.Trim('"'), matchLogins.Value),
                        Convert.ToInt32(matchTryAnswers.Value));
                }
            }
        }
    ...
    public static void EnterBirthDay(out DateTime dateTime)
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("Введите Вашу дату рождения в формате месяц.день.год: ");
            try
            {
    ...
    public void SaveNewAccInFile(DateTime dateTime)
    {
        using (StreamWriter writer = new StreamWriter(PathAccs, false, Encoding.Default))
        {
            foreach (var item in Accs)
                writer.WriteLine($"{item.Key} - {item.Value} - {dateTime.ToShortDateString()}");
        }
    }
    ...
    public void ViewTop20()
    {
        ...
        do
        {
            ...
            Console.WriteLine("\nДля просмотра ТОП-20 смешанных викторин введите слово \"miks\".");
            Console.Write("\nВвод: ");
            try
            {
    ...
    public void ShowLoginOrRegister() // первый вызываемый метод меню.
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("\t\tДобро пожаловать на Викторину от Бахы! ");
    ...
    public void ShowMenuAllQuizzez()
    {
        ...
        do
        {
            ...
            Console.WriteLine(
                "\nДля старта викторины со случайными вопросами из всех викторин введите слово \"miks\".");
            Console.Write("\nВвод: ");
            try
            {
                string? choice = Console.ReadLine();
