...
public class Account
{
    ...
    {
        using (FileStream fileStream = new FileStream(Menu.PathAccs, FileMode.OpenOrCreate, FileAccess.ReadWrite,
                   FileShare.ReadWrite))
        {
            using (StreamReader reader = new StreamReader(fileStream, Encoding.Default))
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (currentLine.Contains($"{Login}") == true)
                    {
                        ...
                        BirthDateTime = Convert.ToDateTime(matchBirth.ToString());
                        break;
                    }
                }
            }
    ...
    public void ChangePassword()
    {
        ...
        SaveAccauntDataInFile(oldPassword, BirthDateTime);
    ...
    private void SaveAccauntDataInFile(string? oldPassword, DateTime oldDateTime)
    {
                   ...
                   FileShare.ReadWrite))
        {
            ...
            if (tmp.Count == 0)
            {
                int deleteIndex = Array.FindIndex(tmp.ToArray(),
                s => s == $"{Login} - {oldPassword} - {oldDateTime.ToShortDateString()}");
                tmp.RemoveAt(deleteIndex);
                tmp.Add($"{Login} - {Password} - {BirthDateTime.ToShortDateString()}");
            }
    ...
    public void StartQuiz(int choiceQuiz, AllQuizzez myAllQuizzez,
        Dictionary<KeyValuePair<string, string>, int> statistics)
    {
        Console.Clear();
        ...
        while (i < myAllQuizzez.AllQuizzezList[choiceQuiz].QuizList.Count)
        {
            ...
            {
                Console.WriteLine(
                    $"{j} - {myAllQuizzez.AllQuizzezList[choiceQuiz].QuizList[i].PossibleOptionsList[j]}");
            }

            Console.WriteLine("\nВведите номер правильного ответа.");
            Console.WriteLine(
                "Если их несколько, введите номера подряд без пробелов и запятых и нажмите Enter, иначе ответ не засчитается.");
            Console.Write("\nВвод: ");
        ...
        KeyValuePair<string, string> del =
            new KeyValuePair<string, string>(myAllQuizzez.AllQuizzezList[choiceQuiz].QuizName, Login);
        statistics.Remove(del);
        statistics.Add(new KeyValuePair<string, string>(myAllQuizzez.AllQuizzezList[choiceQuiz].QuizName, Login),
            countRightQuestions);
        ...
        Console.WriteLine(
            $"Количество правильных ответов: {countRightQuestions} из {myAllQuizzez.AllQuizzezList[choiceQuiz].QuizList.Count}.");
        Console.WriteLine();
    ...
    public void ShowMyPlaceInCurrentQuiz(Dictionary<KeyValuePair<string, string>, int> statistics, string quizName)
    {
        ...
        Console.WriteLine(
            $"Ваше место среди всех участников данной викторины: {++indexMyPlace} из {currentQuizStats.Count()}");
        Console.ReadKey();
    }
    ...
    public void ChangeDateTimeBirthDay()
    {
        ...
        DateTime oldBirthDay = BirthDateTime;
        BirthDateTime = newDateTimeBirthDay;
